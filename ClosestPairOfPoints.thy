...
fun cpop_set :: "(point * point) \<Rightarrow> point list \<Rightarrow> bool" where
  "cpop_set (p\<^sub>0, p\<^sub>1) ps \<longleftrightarrow> (
    p\<^sub>0 \<in> set ps \<and> p\<^sub>1 \<in> set ps \<and> (distinct ps \<longrightarrow>  p\<^sub>0 \<noteq> p\<^sub>1) \<and> (\<not> distinct ps \<longrightarrow> p\<^sub>0 = p\<^sub>1 \<and> p\<^sub>0 \<in> set (dups ps))
  )"
...
lemma distinct_pairwise:
  assumes "distinct xs"
  shows "(\<forall>i < length xs. \<forall>j < length xs. i \<noteq> j \<longrightarrow> P (xs!i) (xs!j)) \<longleftrightarrow> (\<forall>x \<in> set xs. \<forall>y \<in> set xs. x \<noteq> y \<longrightarrow> P x y)"
  by (metis (no_types, hide_lams) assms distinct_conv_nth in_set_conv_nth)

lemma cpop_dist_iff_cpop_distinct_dist:
  assumes "distinct ps"
  shows "cpop_dist (p\<^sub>0, p\<^sub>1) ps \<longleftrightarrow> cpop_distinct_dist (p\<^sub>0, p\<^sub>1) ps"
  using assms distinct_pairwise[of ps "(\<lambda>x y. dist p\<^sub>0 p\<^sub>1 \<le> dist x y)"] by simp

lemma cpop_distinct_dist_id:
  assumes "cpop_distinct_dist (c\<^sub>0, c\<^sub>1) ps" "\<forall>p \<in> set ps. dist c\<^sub>0 c\<^sub>1 \<le> dist p\<^sub>0 p"
  shows "cpop_distinct_dist (c\<^sub>0, c\<^sub>1) (p\<^sub>0 # ps)"
