...
lemma find_closest_dist:
  ...
  apply (induction p\<^sub>0 ps arbitrary: \<delta> c rule: find_closest.induct)
    apply (auto simp: Let_def less_imp_le less_le_trans split: prod.splits if_splits)
  apply fastforce+
  done
...
proof (induction p \<delta> cs arbitrary: \<delta>\<^sub>c c rule: find_closest_\<delta>.induct)
  ...
  proof (rule ccontr)
    ...
    hence "\<forall>c \<in> set ?cs. \<delta> \<le> dist p c"
      using dist_snd_le order_trans apply (auto split: prod.splits) by fastforce+
    thus False
...
proof -
  obtain \<Delta>' C\<^sub>0' C\<^sub>1' where \<Delta>'_def: "(\<Delta>', C\<^sub>0', C\<^sub>1') = (if \<delta>\<^sub>L < \<delta>\<^sub>R then (\<delta>\<^sub>L, p\<^sub>0\<^sub>L, p\<^sub>1\<^sub>L) else (\<delta>\<^sub>R, p\<^sub>0\<^sub>R, p\<^sub>1\<^sub>R))"
    by metis
  define ps' where ps'_def: "ps' = filter (\<lambda>p. (\<bar>real_of_int (fst p) - l\<bar>) \<le> \<Delta>') ps"
...
lemma set_band_filter_aux:
  ...
  assumes "ps' = filter (\<lambda>p. \<bar>real_of_int (fst p) - l\<bar> \<le> \<delta>) (ps :: point list)"
