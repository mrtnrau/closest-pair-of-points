...
fun find_closest_code :: "point \<Rightarrow> point list \<Rightarrow> (real * point)" where
...
proof (induction p ps arbitrary: \<delta> c \<delta>' c' rule: find_closest.induct)
  ...
  obtain \<delta>\<^sub>1 p\<^sub>1 \<delta>\<^sub>1' p\<^sub>1' where \<delta>\<^sub>1_def: "(\<delta>\<^sub>1, p\<^sub>1) = find_closest p (p\<^sub>2 # ps)"
  ...
  hence "\<delta>\<^sub>0 < \<delta>\<^sub>1 \<longleftrightarrow> \<delta>\<^sub>0' < \<delta>\<^sub>1'"
    using find_closest_dist_eq[of "p\<^sub>2 # ps" \<delta>\<^sub>1 p\<^sub>1 p]
          find_closest_code_dist_eq[of "p\<^sub>2 # ps" \<delta>\<^sub>1' p\<^sub>1' p]
...
fun closest_pair_bf_code :: "point list \<Rightarrow> (real * point * point)" where
...
lemma closest_pair_bf_eq_code:
  ...
  using assms
proof (induction ps arbitrary: \<delta> c\<^sub>0 c\<^sub>1 \<delta>' c\<^sub>0' c\<^sub>1' rule: closest_pair_bf_code.induct)
  case (4 p\<^sub>0 p\<^sub>2 p\<^sub>3 ps)
  let ?ps = "p\<^sub>2 # p\<^sub>3 # ps"
  obtain \<delta>\<^sub>c c\<^sub>0 c\<^sub>1 \<delta>\<^sub>c' c\<^sub>0' c\<^sub>1' where \<delta>\<^sub>c_def: "(\<delta>\<^sub>c, c\<^sub>0, c\<^sub>1) = closest_pair_bf ?ps"
    "(\<delta>\<^sub>c', c\<^sub>0', c\<^sub>1') = closest_pair_bf_code ?ps"
    by (metis prod_cases3)
  obtain \<delta>\<^sub>p p\<^sub>1 \<delta>\<^sub>p' p\<^sub>1' where \<delta>\<^sub>p_def: "(\<delta>\<^sub>p, p\<^sub>1) = find_closest p\<^sub>0 ?ps"
  ...
  moreover have "\<delta>\<^sub>p = dist p\<^sub>0 p\<^sub>1" "\<delta>\<^sub>p' = dist_code p\<^sub>0 p\<^sub>1'"
    using defs find_closest_dist_eq find_closest_code_dist_eq by blast+
  ultimately have "\<delta>\<^sub>c \<le> \<delta>\<^sub>p \<longleftrightarrow> \<delta>\<^sub>c' \<le> \<delta>\<^sub>p'"
...
fun find_closest_\<delta>_code :: "point \<Rightarrow> real \<Rightarrow> point list \<Rightarrow> (real * point)" where
  "find_closest_\<delta>_code p \<delta> [] = undefined"
| "find_closest_\<delta>_code p \<delta> [c] = (dist_code p c, c)"
| "find_closest_\<delta>_code p \<delta> (c\<^sub>0 # cs) = (
    ...
    if \<delta> \<le> (snd c\<^sub>0 - snd p)\<^sup>2 then
      ...
      let (\<delta>\<^sub>1, c\<^sub>1) = find_closest_\<delta>_code p (min \<delta> \<delta>\<^sub>0) cs in
...
lemma find_closest_\<delta>_code_dist_eq:
  "0 < length cs \<Longrightarrow> (\<delta>\<^sub>c, c) = find_closest_\<delta>_code p \<delta> cs \<Longrightarrow> \<delta>\<^sub>c = dist_code p c"
proof (induction p \<delta> cs arbitrary: \<delta>\<^sub>c c rule: find_closest_\<delta>_code.induct)
  ...
  next
    assume A: "\<not> \<delta> \<le> (snd c\<^sub>0 - snd p)\<^sup>2"
    define \<delta>\<^sub>0 where \<delta>\<^sub>0_def: "\<delta>\<^sub>0 = dist_code p c\<^sub>0"
    obtain \<delta>\<^sub>1 c\<^sub>1 where \<delta>\<^sub>1_def: "(\<delta>\<^sub>1, c\<^sub>1) = find_closest_\<delta>_code p (min \<delta> \<delta>\<^sub>0) (c\<^sub>2 # cs)"
      by (metis surj_pair)
    note defs = \<delta>\<^sub>0_def \<delta>\<^sub>1_def
...
lemma find_closest_\<delta>_eq_code:
  ...
  shows "c = c'"
  using assms
proof (induction p \<delta> ps arbitrary: \<delta>' p\<^sub>0 p\<^sub>1 \<delta>\<^sub>c c \<delta>\<^sub>c' c' rule: find_closest_\<delta>.induct)
  case (3 p \<delta> c\<^sub>0 c\<^sub>2 cs)
  define \<delta>\<^sub>0 \<delta>\<^sub>0' where \<delta>\<^sub>0_def: "\<delta>\<^sub>0 = dist p c\<^sub>0" "\<delta>\<^sub>0' = dist_code p c\<^sub>0"
  obtain \<delta>\<^sub>1 c\<^sub>1 \<delta>\<^sub>1' c\<^sub>1' where \<delta>\<^sub>1_def: "(\<delta>\<^sub>1, c\<^sub>1) = find_closest_\<delta> p (min \<delta> \<delta>\<^sub>0) (c\<^sub>2 # cs)"
    "(\<delta>\<^sub>1', c\<^sub>1') = find_closest_\<delta>_code p (min \<delta>' \<delta>\<^sub>0') (c\<^sub>2 # cs)"
    by (metis surj_pair)
  note defs = \<delta>\<^sub>0_def \<delta>\<^sub>1_def
  show ?case
  proof cases
    assume *: "\<delta> \<le> \<bar>snd c\<^sub>0 - snd p\<bar>"
    hence "\<delta>' \<le> (snd c\<^sub>0 - snd p)\<^sup>2"
      using "3.prems"(2,3) by (simp add: dist_eq_dist_code_abs_2)
    thus ?thesis
      using * "3.prems"(4,5) by simp
  next
    assume *: "\<not> \<delta> \<le> \<bar>snd c\<^sub>0 - snd p\<bar>"
    hence A: "\<not> \<delta>' \<le> (snd c\<^sub>0 - snd p)\<^sup>2"
      using "3.prems"(2,3) by (simp add: dist_eq_dist_code_abs_2)
    ...
    hence B: "c\<^sub>1 = c\<^sub>1'"
      using "3.IH"[of \<delta>\<^sub>0 _ _ "min \<delta>' \<delta>\<^sub>0'"] "3.prems"(2,3) * defs
      by (smt length_greater_0_conv list.discI)
    have "\<delta>\<^sub>1 = dist p c\<^sub>1" "\<delta>\<^sub>1' = dist_code p c\<^sub>1'"
      using find_closest_\<delta>_dist_eq find_closest_\<delta>_code_dist_eq defs by blast+
    hence "\<delta>\<^sub>0 \<le> \<delta>\<^sub>1 \<longleftrightarrow> \<delta>\<^sub>0' \<le> \<delta>\<^sub>1'"
...
fun closest_pair_combine_code :: "(real * point * point) \<Rightarrow> point list \<Rightarrow> (real * point * point)" where
...
lemma closest_pair_combine_code_eq:
  ...
  shows "C\<^sub>0 = C\<^sub>0' \<and> C\<^sub>1 = C\<^sub>1'"
  using assms
proof (induction "(\<delta>, c\<^sub>0, c\<^sub>1)" ps arbitrary: \<delta> \<delta>' c\<^sub>0 c\<^sub>1 \<Delta> C\<^sub>0 C\<^sub>1 \<Delta>' C\<^sub>0' C\<^sub>1' rule: closest_pair_combine.induct)
  case (3 \<delta> c\<^sub>0 c\<^sub>1 p\<^sub>0 p\<^sub>2 ps)
  obtain \<delta>\<^sub>p p\<^sub>1 \<delta>\<^sub>p' p\<^sub>1' where \<delta>\<^sub>p_def: "(\<delta>\<^sub>p, p\<^sub>1) = find_closest_\<delta> p\<^sub>0 \<delta> (p\<^sub>2 # ps)"
  ...
  proof (cases "\<delta> \<le> \<delta>\<^sub>p")
    ...
    obtain \<Delta>\<^sub>i C\<^sub>0\<^sub>i C\<^sub>1\<^sub>i \<Delta>\<^sub>i' C\<^sub>0\<^sub>i' C\<^sub>1\<^sub>i' where \<Delta>\<^sub>i_def:
      "(\<Delta>\<^sub>i, C\<^sub>0\<^sub>i, C\<^sub>1\<^sub>i) = closest_pair_combine (\<delta>, c\<^sub>0, c\<^sub>1) (p\<^sub>2 # ps)"
    ...
    hence "C\<^sub>0\<^sub>i = C\<^sub>0\<^sub>i' \<and> C\<^sub>1\<^sub>i = C\<^sub>1\<^sub>i'"
      using "3.hyps"(1)[of "(\<delta>\<^sub>p, p\<^sub>1)" \<delta>\<^sub>p p\<^sub>1] "3.prems"(1,2) True \<delta>\<^sub>p_def by blast
    moreover have "\<Delta> = \<Delta>\<^sub>i" "C\<^sub>0 = C\<^sub>0\<^sub>i" "C\<^sub>1 = C\<^sub>1\<^sub>i"
      using defs(1,3) True "3.prems"(3) apply (auto split: prod.splits) by (metis Pair_inject)+
    moreover have "\<Delta>' = \<Delta>\<^sub>i'" "C\<^sub>0' = C\<^sub>0\<^sub>i'" "C\<^sub>1' = C\<^sub>1\<^sub>i'"
  ...
  next
    ...
    hence C: "\<not> \<delta>' \<le> \<delta>\<^sub>p'"
      by (simp add: "3.prems"(1,2) A B dist_eq_dist_code_le)
    obtain \<Delta>\<^sub>i C\<^sub>0\<^sub>i C\<^sub>1\<^sub>i \<Delta>\<^sub>i' C\<^sub>0\<^sub>i' C\<^sub>1\<^sub>i' where \<Delta>\<^sub>i_def:
      "(\<Delta>\<^sub>i, C\<^sub>0\<^sub>i, C\<^sub>1\<^sub>i) = closest_pair_combine (\<delta>\<^sub>p, p\<^sub>0, p\<^sub>1) (p\<^sub>2 # ps)"
      "(\<Delta>\<^sub>i', C\<^sub>0\<^sub>i', C\<^sub>1\<^sub>i') = closest_pair_combine_code (\<delta>\<^sub>p', p\<^sub>0, p\<^sub>1') (p\<^sub>2 # ps)"
      by (metis prod_cases4)
    note defs = \<delta>\<^sub>p_def \<Delta>\<^sub>i_def
    hence "C\<^sub>0\<^sub>i = C\<^sub>0\<^sub>i' \<and> C\<^sub>1\<^sub>i = C\<^sub>1\<^sub>i'"
      using "3.hyps"(2)[of "(\<delta>\<^sub>p, p\<^sub>1)" \<delta>\<^sub>p p\<^sub>1] A B False \<delta>\<^sub>p_def by blast
    moreover have "\<Delta> = \<Delta>\<^sub>i" "C\<^sub>0 = C\<^sub>0\<^sub>i" "C\<^sub>1 = C\<^sub>1\<^sub>i"
      using defs(1,3) False "3.prems"(3) apply (auto split: prod.splits) by (metis Pair_inject)+
    moreover have "\<Delta>' = \<Delta>\<^sub>i'" "C\<^sub>0' = C\<^sub>0\<^sub>i'" "C\<^sub>1' = C\<^sub>1\<^sub>i'"
      using defs(2,4) C "3.prems"(4) apply (auto split: prod.splits) by (metis Pair_inject)+
    ultimately show ?thesis
      by simp
...
fun combine_code :: "(real * point * point) \<Rightarrow> (real * point * point) \<Rightarrow> real \<Rightarrow> point list \<Rightarrow> (real * point * point)" where
...
proof -
  ...
  define ps' ps'' where ps'_def:
    "ps' = filter (\<lambda>p. \<bar>fst p - l\<bar> \<le> \<Delta>\<^sub>i) ps"
    "ps'' = filter (\<lambda>p. (fst p - l)\<^sup>2 \<le> \<Delta>\<^sub>i') ps"
  obtain \<Delta> C\<^sub>0 C\<^sub>1 \<Delta>' C\<^sub>0' C\<^sub>1' where \<Delta>_def:
    "(\<Delta>, C\<^sub>0, C\<^sub>1) = closest_pair_combine (\<Delta>\<^sub>i, C\<^sub>0\<^sub>i, C\<^sub>1\<^sub>i) ps'"
  ...
  have *: "C\<^sub>0\<^sub>i = C\<^sub>0\<^sub>i'" "C\<^sub>1\<^sub>i = C\<^sub>1\<^sub>i'" "\<Delta>\<^sub>i = dist C\<^sub>0\<^sub>i C\<^sub>1\<^sub>i" "\<Delta>\<^sub>i' = dist_code C\<^sub>0\<^sub>i' C\<^sub>1\<^sub>i'"
    using \<Delta>\<^sub>i_def assms(1,2,3,4) dist_eq_dist_code_lt by (auto split: if_splits)
  hence "ps' = ps''"
    using dist_eq_dist_code_abs_1 ps'_def by simp
  ...
  moreover have "C\<^sub>0 = c\<^sub>0" "C\<^sub>1 = c\<^sub>1"
    using assms(5) defs(1,3,5) apply (auto split: prod.splits if_splits) by (metis prod.inject)+
  moreover have "C\<^sub>0' = c\<^sub>0'" "C\<^sub>1' = c\<^sub>1'"
    using assms(6) defs(2,4,6) apply (auto split: prod.splits if_splits) by (metis prod.inject)+
  ultimately show ?thesis
    by blast
qed
...
function closest_pair_rec_code :: "point list \<Rightarrow> (point list * real * point * point)" where
...
lemma closest_pair_rec_ys_eq:
  assumes "1 < length xs"
  assumes "(ys, \<delta>, c\<^sub>0, c\<^sub>1) = closest_pair_rec xs"
  assumes "(ys', \<delta>', c\<^sub>0', c\<^sub>1') = closest_pair_rec_code xs"
  shows "ys = ys'"
  using assms
proof (induction xs arbitrary: ys \<delta> c\<^sub>0 c\<^sub>1 ys' \<delta>' c\<^sub>0' c\<^sub>1' rule: length_induct)
  case (1 xs)
  let ?n = "length xs"
  show ?case
  proof (cases "?n \<le> 3")
  ...
  next
    ...
    obtain YS\<^sub>L \<Delta>\<^sub>L C\<^sub>0\<^sub>L C\<^sub>1\<^sub>L YS\<^sub>L' \<Delta>\<^sub>L' C\<^sub>0\<^sub>L' C\<^sub>1\<^sub>L' where YSC\<^sub>0\<^sub>1\<^sub>L_def:
      "(YS\<^sub>L, \<Delta>\<^sub>L, C\<^sub>0\<^sub>L, C\<^sub>1\<^sub>L) = closest_pair_rec XS\<^sub>L"
      "(YS\<^sub>L', \<Delta>\<^sub>L', C\<^sub>0\<^sub>L', C\<^sub>1\<^sub>L') = closest_pair_rec_code XS\<^sub>L"
      using prod.collapse by metis
    obtain YS\<^sub>R \<Delta>\<^sub>R C\<^sub>0\<^sub>R C\<^sub>1\<^sub>R YS\<^sub>R' \<Delta>\<^sub>R' C\<^sub>0\<^sub>R' C\<^sub>1\<^sub>R' where YSC\<^sub>0\<^sub>1\<^sub>R_def:
      "(YS\<^sub>R, \<Delta>\<^sub>R, C\<^sub>0\<^sub>R, C\<^sub>1\<^sub>R) = closest_pair_rec XS\<^sub>R"
    ...
    define YS YS' where YS_def:
      ...
      "YS' = merge (\<lambda>p. snd p) YS\<^sub>L' YS\<^sub>R'"
    obtain \<Delta> C\<^sub>0 C\<^sub>1 \<Delta>' C\<^sub>0' C\<^sub>1' where C\<^sub>0\<^sub>1_def:
      "(\<Delta>, C\<^sub>0, C\<^sub>1) = combine (\<Delta>\<^sub>L, C\<^sub>0\<^sub>L, C\<^sub>1\<^sub>L) (\<Delta>\<^sub>R, C\<^sub>0\<^sub>R, C\<^sub>1\<^sub>R) L YS"
...
proof (induction xs arbitrary: ys \<delta> c\<^sub>0 c\<^sub>1 ys' \<delta>' c\<^sub>0' c\<^sub>1' rule: length_induct)
  ...
  next
    ...
    moreover have "\<Delta>\<^sub>L = dist C\<^sub>0\<^sub>L C\<^sub>1\<^sub>L" "\<Delta>\<^sub>R = dist C\<^sub>0\<^sub>R C\<^sub>1\<^sub>R"
      using defs \<open>1 < length XS\<^sub>L\<close> \<open>1 < length XS\<^sub>R\<close> closest_pair_rec_dist_eq by blast+
    ...
    moreover have "(YS, \<Delta>, C\<^sub>0, C\<^sub>1) = closest_pair_rec xs"
      using False closest_pair_rec_simps defs(1,2,3,5,7,9)
      by (auto simp: Let_def split: prod.split)
    moreover have "(YS', \<Delta>', C\<^sub>0', C\<^sub>1') = closest_pair_rec_code xs"
      using False closest_pair_rec_code_simps defs(1,2,4,6,8,10)
...
proof (induction ps rule: induct_list012)
  case (3 x y zs)
  obtain ys \<delta> c\<^sub>0 c\<^sub>1 ys' \<delta>' c\<^sub>0' c\<^sub>1' where *:
    "(ys, \<delta>, c\<^sub>0, c\<^sub>1) = closest_pair_rec (sortX (x # y # zs))"
